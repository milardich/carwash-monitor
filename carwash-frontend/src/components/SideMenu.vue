...
<template>
    <aside id="default-sidebar"
        ...
        :class="props.open ? 'translate-x-0' : '-translate-x-full'" aria-label="Sidebar">
        <div class="h-full px-3 py-4 overflow-y-auto text-gray-light side-menu flex flex-col">
            <!-- Header row with close button (mobile only) -->
            <div class="flex items-center justify-between mb-4 lg:hidden px-1">
                <span class="text-lg font-semibold text-white">Menu</span>
                <button @click="emit('close')"
                    class="p-2 rounded-lg text-white/80 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-white/40"
            ...
            <ul class="space-y-2 font-medium">
                <li>
                    <RouterLink to="/dashboard" @click="handleNav">
                        <span class="side-menu-item flex items-center p-2 rounded-lg group"
                            :class="menuItemColor('dashboard')">
                ...
                <li>
                    <RouterLink to="/statistics" @click="handleNav">
                        <span class="side-menu-item flex items-center p-2 rounded-lg group"
                            :class="menuItemColor('statistics')">
                            <svg class="flex-shrink-0 w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                ...
                <li>
                    <RouterLink to="/control-panel" @click="handleNav">
                        <span class="side-menu-item flex items-center p-2 rounded-lg group"
